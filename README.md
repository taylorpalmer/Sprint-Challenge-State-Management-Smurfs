...
Context API helps simplify state management when an application has many components and they are deeply nested and depend on a sigle piece of data.

- [ ] In your own words, describe `actions`, `reducers` and the `store` and their role in Redux. What does each piece do? Why is the store known as a 'single source of truth' in a redux application?

Actions cause changes in state. Reducers pull all of the actions together with the initial state. The store holds the initial state and is immutable. The store state is copied and that is changed.

- [ ] What is the difference between Application state and Component state? When would be a good time to use one over the other?

Application state is immutable and covers the whole application. Component state can change and should be used for something that only that one component uses such as form state.

- [ ] Describe `redux-thunk`, what does it allow us to do? How does it change our `action-creators`?

Redux-thunk allows us to pass funtions into our action creators.

- [ ] What is your favorite state management system you've learned and this sprint? Please explain why!

I like Redux because you think of everything that you need the application to do, you create corresponding actions and then pass them to the reducer to use in the app.
